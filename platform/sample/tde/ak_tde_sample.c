...
char ac_option_hint[  ][ LEN_HINT ] = {                                         //操作提示数组
    ...
    "\"[PIC-WIDTH] [PIC-HEIGHT] [RECT-TOP] [RECT-LEFT] [RECT-WIDTH] [RECT-HEIGHT]\" 源图宽高和进行处理坐标参数" ,
    "\"[PIC-WIDTH] [PIC-HEIGHT]\" 背景图宽高" ,
    ...
    "[NUM] logo图层格式 0:RGB565 1:RGB888 2:BGR565 3:BGR888 4:YUV420P 5:YUV420SP 6:ARGB8888 7:RGBA8888 8:ABGR8888 9:BGRA8888 10:TILED32X4" ,
    "" ,
};

double af_format_byte[  ] = { 2 , 3 , 2 , 3 , 1.5 , 1.5 , 4 , 4 , 4 , 4 , 0 };  //图形格式占用的字节数
struct ak_tde_layer tde_layer_src, tde_layer_bg, tde_layer_fb;
...
struct option option_long[ ] = {
    ...
    { "rect-s"            , required_argument , NULL , 'g' } ,                  //"\"[PIC-WIDTH] [PIC-HEIGHT] [RECT-TOP] [RECT-LEFT] [RECT-WIDTH] [RECT-HEIGHT]\" 源图宽高和进行处理坐标参数" ,
    { "rect-bg"           , required_argument , NULL , 'i' } ,                  //"\"[PIC-WIDTH] [PIC-HEIGHT]\" 背景图宽高" ,
    ...
    { "format-logo"       , required_argument , NULL , 'l' } ,                  //"[NUM] logo图层格式 0:RGB565 1:RGB888 2:BGR565 3:BGR888 4:YUV420P 5:YUV420SP 6:ARGB8888 7:RGBA8888 8:ABGR8888 9:BGRA8888 10:TILED32X4" ,
    { 0                   , 0                 , 0    ,  0  } ,                  //"" ,
 };
    ...
    while((i_option = getopt_long(argc , argv , ac_option_short , option_long , NULL)) > 0) {
        switch(i_option) {
            ...
            case 'i' :                                                          //rect-bg 目标图层参数设置
                ...
                tde_layer_bg.pos_width = tde_layer_bg.width;
                tde_layer_bg.pos_height = tde_layer_bg.height;
            ...
            case 'j' :
                ...
                break;
            case 'k' :
                tde_layer_fb.format_param = atoi(optarg);
                break;
            case 'l' :
...
int main( int argc, char **argv )
{
    ...
    tde_layer_fb.format_param     = GP_FORMAT_RGB888;
    ...
    printf("TDE lib version is [%s]\n", ak_tde_get_version());                  //print TDE version

    ak_tde_open( );
    ak_gui_open( &fb_fix_screeninfo_param, &fb_var_screeninfo_param, &p_vaddr_fb);
    tde_layer_fb.width            = fb_var_screeninfo_param.xres;
    tde_layer_fb.height           = fb_var_screeninfo_param.yres;
    tde_layer_fb.pos_left         = 0;
    tde_layer_fb.pos_top          = 0;
    tde_layer_fb.pos_width        = fb_var_screeninfo_param.xres;
    tde_layer_fb.pos_height       = fb_var_screeninfo_param.yres;
    test_tde( );                                                                //tde图形测试
...
int test_tde( void )                                                            //tde图形操作测试
{
    ...
    int i_dmasize_src = tde_layer_src.width * tde_layer_src.height * af_format_byte[ tde_layer_src.format_param ];
    int i_dmasize_bg  = tde_layer_bg.width * tde_layer_bg.height * af_format_byte[ tde_layer_bg.format_param ];
    ...
    i_filesize_src = get_file_size( pc_file_src );
    ...
    tde_layer_fb.phyaddr = fb_fix_screeninfo_param.smem_start;
    ...
    if( ( i_filesize_bg != SIZE_ERROR ) && ( i_filesize_bg != 0 ) ) {           //背景图片
        ...
        fclose( pFILE );
        ak_tde_opt_scale( &tde_layer_bg, &tde_layer_fb );                       //贴背景图片
    }
    else {
    ...
    if ( ( i_filesize_src != i_dmasize_src ) || ( i_filesize_bg != i_dmasize_bg ) ) {
        printf( "i_filesize_src= %u i_dmasize_src= %u i_filesize_bg= %u i_dmasize_bg= %u\n",
                i_filesize_src, i_dmasize_src, i_filesize_bg, i_dmasize_bg );
    }

    ak_mem_dma_vaddr2paddr( p_vaddr_src , ( unsigned long * )&tde_layer_src.phyaddr );
    tde_layer_fb.pos_width = tde_layer_src.pos_width;
    tde_layer_fb.pos_height = tde_layer_src.pos_height;
    tde_layer_fb.pos_left = tde_layer_src.pos_left;
    tde_layer_fb.pos_top = tde_layer_src.pos_top;                               //设置目标图层参数
    tde_layer_src.pos_left = 0;
    tde_layer_src.pos_top = 0;
    tde_layer_src.pos_width = tde_layer_src.width;
    tde_layer_src.pos_height = tde_layer_src.height;                            //设置源图层参数

    tde_cmd_param.tde_layer_src = tde_layer_src;
    tde_cmd_param.tde_layer_dst = tde_layer_fb;
