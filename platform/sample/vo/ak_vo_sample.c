...
int screen_flag    = 0;                                                         //mipi屏幕
char *data_file = "/mnt/yuv";
char *logo_file = "/mnt/anyka.logo.577.160.rgb";
int display_num = 4;
AK_GP_FORMAT data_format = GP_FORMAT_YUV420P;
AK_GP_FORMAT out_format  = GP_FORMAT_RGB888;
...
int main(int argc, char **argv)
{
    ...
    ak_print_normal_ex(MODULE_ID_VO, "----- %s -----\n", argv[0]);
    ak_print_normal_ex(MODULE_ID_VO, "Vo lib version : [%s]\n", ak_vo_get_version());    //print the version

    /* parse the option */
    if( parse_option( argc, argv ) == AK_FALSE )            //解释和配置选项
    {
        return AK_FAILED;                                           //打印帮助后退兿
    }
